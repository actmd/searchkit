import { Component, PropTypes, cloneElement, createElement, isValidElement } from 'react';
...
var EventEmitter = (function () {
  ...
  EventEmitter.prototype.trigger = function () {
    ...
    forEach(this.listeners, function (fn) {
      fn.apply(null, args);
    });
  };
  return EventEmitter;
...
var update$2 = require("react-addons-update");
var LevelState = (function (_super) {
  ...
  LevelState.prototype.remove = function (level, val) {
    return this.create(update$2(this.getValue(), (_a = {},
...
function ChildrenBucket(key, type) {
  ...
  return AggsContainer(key, { children: { type: type } }, childAggs);
}
function FilterBucket(key, filter$$1) {
  var childAggs = [];
  for (var _i = 2; _i < arguments.length; _i++) {
    childAggs[_i - 2] = arguments[_i];
  }
  return AggsContainer(key, { filter: filter$$1 }, childAggs);
}
function NestedBucket(key, path) {
  ...
  return AggsContainer(key, { nested: { path: path } }, childAggs);
}
function SignificantTermsBucket(key, field, options) {
  ...
  return AggsContainer(key, { significant_terms: assign({ field: field }, options) }, childAggs);
}
function GeohashBucket(key, field, options) {
  ...
  return AggsContainer(key, { geohash_grid: assign({ field: field }, options) }, childAggs);
}
function HistogramBucket(key, field, options) {
  ...
  return AggsContainer(key, { histogram: assign({ field: field }, options) }, childAggs);
}

function FieldMetricFactory(metricOp) {
  return function (key, field) {
    return AggsContainer(key, (_a = {},
      _a[metricOp] = { field: field },
      _a));
    var _a;
  };
}
var CardinalityMetric = FieldMetricFactory("cardinality");
var MinMetric = FieldMetricFactory("min");
var MaxMetric = FieldMetricFactory("max");
var AvgMetric = FieldMetricFactory("avg");
var SumMetric = FieldMetricFactory("sum");
var StatsMetric = FieldMetricFactory("stats");
function TopHitsMetric(key, top_hits) {
  return AggsContainer(key, { top_hits: top_hits });
}
function GeoBoundsMetric(key, field, options) {
  if (options === void 0) { options = {}; }
  return AggsContainer(key, { geo_bounds: assign({ field: field }, options) });
}

function AvgBucketPipeline(key, buckets_path) {
  return AggsContainer(key, { buckets_path: buckets_path });
}
...
var allowedOptions$1 = ["score_mode", "inner_hits"];
function HasParentQuery(parent_type, query, options) {
  if (options === void 0) { options = {}; }
  return {
    has_parent: assign({
      parent_type: parent_type, query: query
    }, pick(options, allowedOptions$1))
  };
}

var allowedOptions$2 = ["score_mode", "inner_hits"];
function NestedQuery(path, query, options) {
  if (options === void 0) { options = {}; }
  return {
    nested: assign({
      path: path, query: query
    }, pick(options, allowedOptions$2))
  };
}
...
var update$3 = require("react-addons-update");
var ImmutableQuery = (function () {
  ...
  ImmutableQuery.prototype.addFilter = function (key, filter$$1) {
    return this.update({
      filters: { $push: [filter$$1] },
      filtersMap: { $merge: (_a = {}, _a[key] = filter$$1, _a) }
    });
    var _a;
  };
  ...
  ImmutableQuery.prototype.getFilters = function (keys$$1) {
    if (keys$$1 === void 0) { keys$$1 = []; }
    return this.getFiltersWithoutKeys(keys$$1);
  };
  ImmutableQuery.prototype._getFilters = function (keys$$1, method$$1) {
    keys$$1 = [].concat(keys$$1);
    var filters = values(method$$1(this.index.filtersMap || {}, keys$$1));
    return BoolMust(filters);
  };
  ImmutableQuery.prototype.getFiltersWithKeys = function (keys$$1) {
    return this._getFilters(keys$$1, pick);
  };
  ImmutableQuery.prototype.getFiltersWithoutKeys = function (keys$$1) {
    return this._getFilters(keys$$1, omit);
  };
  ImmutableQuery.prototype.setSize = function (size$$1) {
    return this.update({ $merge: { size: size$$1 } });
  };
  ...
  ImmutableQuery.prototype.update = function (updateDef) {
    return new ImmutableQuery(update$3(this.index, updateDef));
  };
...
var FieldContext = (function () {
  function FieldContext(fieldOptions) {
...
var NestedFieldContext = (function (_super) {
  ...
  NestedFieldContext.prototype.wrapAggregations = function () {
    ...
    return [NestedBucket.apply(void 0, ["inner",
      this.fieldOptions.options.path].concat(aggregations))];
  };
  NestedFieldContext.prototype.wrapFilter = function (filter$$1) {
    return NestedQuery(this.fieldOptions.options.path, filter$$1, this.fieldOptions.options);
  };
...
var ChildrenFieldContext = (function (_super) {
  ...
  ChildrenFieldContext.prototype.wrapAggregations = function () {
    ...
    return [ChildrenBucket.apply(void 0, ["inner",
      this.fieldOptions.options.childType].concat(aggregations))];
  };
  ChildrenFieldContext.prototype.wrapFilter = function (filter$$1) {
    return HasChildQuery(this.fieldOptions.options.childType, filter$$1, this.fieldOptions.options);
  };
...
var FieldContextFactory = function (fieldOptions) {
...
var CheckboxFilterAccessor = (function (_super) {
  ...
  CheckboxFilterAccessor.prototype.buildOwnQuery = function (query) {
    ...
    return query
      .setAggs(FilterBucket(this.uuid, filters));
  };
...
var FacetAccessor = (function (_super) {
  ...
  function FacetAccessor(key, options) {
    ...
    _this.fieldContext = FieldContextFactory(_this.options.fieldOptions);
    return _this;
  }
  ...
  FacetAccessor.prototype.getBuckets = function () {
    ...
    var inIndex = function (filter$$1) { return !!keyIndex[filter$$1]; };
    var missingFilters = [];
    forEach(this.state.getValue(), function (filterKey) {
      if (keyIndex[filterKey]) {
        var filter$$1 = keyIndex[filterKey];
  ...
  FacetAccessor.prototype.buildSharedQuery = function (query) {
    ...
    var filterTerms = map(filters, function (filter$$1) {
      return _this.fieldContext.wrapFilter(TermQuery(_this.key, filter$$1));
    });
    var selectedFilters = map(filters, function (filter$$1) {
      return {
        name: _this.options.title || _this.translate(_this.key),
        value: _this.translate(filter$$1),
        id: _this.options.id,
        remove: function () { return _this.state = _this.state.remove(filter$$1); }
      };
    });
  ...
  FacetAccessor.prototype.buildOwnQuery = function (query) {
    if (!this.loadAggregations) {
      ...
      return query
        .setAggs(FilterBucket.apply(void 0, [this.uuid,
          query.getFiltersWithoutKeys(excludedKey)].concat(this.fieldContext.wrapAggregations(TermsBucket(this.key, this.key, omitBy({
          size: this.size,
          order: this.getOrder(),
          include: this.options.include,
          exclude: this.options.exclude,
          min_doc_count: this.options.min_doc_count
        }, isUndefined)), CardinalityMetric(this.key + "_count", this.key)))));
    }
  };
...
var noopQueryAccessor = new NoopQueryAccessor(null);
...
var QueryAccessor = (function (_super) {
  ...
  QueryAccessor.prototype.buildSharedQuery = function (query) {
    ...
    if (queryStr) {
      var queryBuilder = this.options.queryBuilder || SimpleQueryString;
...
var mapValues$2 = require("lodash/mapValues");
var zipObject$2 = require("lodash/zipObject");
var constant$2 = require("lodash/constant");
...
var HierarchicalFacetAccessor = (function (_super) {
  ...
  HierarchicalFacetAccessor.prototype.buildSharedQuery = function (query) {
    var _this = this;
    forEach(this.options.fields, function (field, i) {
      ...
      if (isLeaf) {
        var selectedFilters = map(filters, function (filter$$1) {
          return {
            ...
            value: _this.translate(filter$$1),
            remove: function () {
              _this.state = _this.state.remove(i, filter$$1);
            }
          };
        });
  ...
  HierarchicalFacetAccessor.prototype.buildOwnQuery = function (query) {
    ...
    var lvlAggs = compact(map(this.options.fields, function (field, i) {
      if (_this.state.levelHasFilters(i - 1) || i == 0) {
        return FilterBucket(field, query.getFiltersWithKeys(take(_this.uuids, i)), TermsBucket(field, field, omitBy({
          size: _this.options.size, order: _this.getOrder()
        }, isUndefined)));
      }
    }));
    var aggs = FilterBucket.apply(void 0, [this.options.id,
...
var NumericOptionsAccessor$$1 = (function (_super) {
  __extends(NumericOptionsAccessor$$1, _super);
  function NumericOptionsAccessor$$1(key, options) {
    var _this = _super.call(this, key) || this;
    _this.state = new ArrayState();
    _this.options = options;
    _this.options.options = Utils.computeOptionKeys(options.options, ["from", "to"], "all");
    _this.options.fieldOptions = _this.options.fieldOptions || { type: "embedded" };
    _this.options.fieldOptions.field = _this.options.field;
    _this.fieldContext = FieldContextFactory(_this.options.fieldOptions);
    return _this;
  }
  NumericOptionsAccessor$$1.prototype.getDefaultOption = function () {
    return find(this.options.options, function (it) { return isUndefined(it.from) && isUndefined(it.to); });
  };
  NumericOptionsAccessor$$1.prototype.getSelectedOptions = function () {
    var keys$$1 = this.state.getValue();
    return filter(this.options.options, function (opt) { return includes(keys$$1, opt.key); });
  };
  NumericOptionsAccessor$$1.prototype.getSelectedOrDefaultOptions = function () {
  ...
  NumericOptionsAccessor$$1.prototype.setOptions = function (titles) {
    if (titles.length === 1) {
      this.state = this.state.clear();
      this.toggleOption(titles[0]);
    }
    else {
      var keys$$1 = map(filter(this.options.options, function (opt) { return includes(titles, opt.title); }), "key");
      this.state = this.state.setValue(keys$$1);
      this.searchkit.performSearch();
    }
  };
  NumericOptionsAccessor$$1.prototype.toggleOption = function (key) {
  ...
  NumericOptionsAccessor$$1.prototype.getBuckets = function () {
  ...
  NumericOptionsAccessor$$1.prototype.getDocCount = function () {
    return this.getAggregations([
      this.uuid,
      this.fieldContext.getAggregationPath(),
      "doc_count"
  ...
  NumericOptionsAccessor$$1.prototype.emptyOptionsFilter = function (option) {
    return option.doc_count > 0;
  };
  NumericOptionsAccessor$$1.prototype.buildSharedQuery = function (query) {
    var _this = this;
    var filters = this.getSelectedOptions();
    var filterRanges = map(filters, function (filter$$1) {
      return _this.fieldContext.wrapFilter(RangeQuery(_this.options.field, {
        gte: filter$$1.from, lt: filter$$1.to
      }));
    });
    var selectedFilters = map(filters, function (filter$$1) {
      return {
        name: _this.translate(_this.options.title),
        value: _this.translate(filter$$1.title),
        id: _this.options.id,
        remove: function () { return _this.state = _this.state.remove(filter$$1.key); }
      };
    });
  ...
  NumericOptionsAccessor$$1.prototype.getRanges = function () {
    return compact(map(this.options.options, function (range$$1) {
      return omitBy({
        key: range$$1.title,
        from: range$$1.from,
        to: range$$1.to
  ...
  NumericOptionsAccessor$$1.prototype.buildOwnQuery = function (query) {
    return query.setAggs(FilterBucket.apply(void 0, [this.uuid,
      query.getFiltersWithoutKeys(this.uuid)].concat(this.fieldContext.wrapAggregations(RangeBucket(this.key, this.options.field, this.getRanges())))));
  };
  return NumericOptionsAccessor$$1;
}(FilterBasedAccessor));
...
var SortingAccessor = (function (_super) {
  ...
  function SortingAccessor(key, options) {
    ...
    _this.state = new ValueState();
    _this.options = options;
    _this.options.options = Utils.computeOptionKeys(_this.options.options, ["field", "order"], "none");
    return _this;
  }
...
var NestedFacetAccessor = (function (_super) {
  ...
  NestedFacetAccessor.prototype.buildSharedQuery = function (query) {
    ...
    var filterTerms = map(levelFilters, function (filter$$1, i) {
      var value$$1 = filter$$1[0];
      var isLeaf = i === lastIndex;
      var subField = isLeaf ? ".value" : ".ancestors";
      return TermQuery(_this.options.field + subField, value$$1);
    });
    if (filterTerms.length > 0) {
  ...
  NestedFacetAccessor.prototype.getTermAggs = function () {
    ...
    return TermsBucket("children", valueField, { size: 0, order: orderMetric }, subAggs);
    var _a, _b;
  };
  NestedFacetAccessor.prototype.buildOwnQuery = function (query) {
    ...
    var addLevel = function (level, ancestors) {
      ...
      lvlAggs.push(FilterBucket("lvl" + level, BoolMust([TermQuery(levelField, level + startLevel)].concat(ancestors)), termAggs));
    };
    addLevel(0);
    ...
    return query.setAggs(FilterBucket(this.key, query.getFiltersWithoutKeys(this.uuid), NestedBucket.apply(void 0, ["children",
...
var RangeAccessor = (function (_super) {
  ...
  RangeAccessor.prototype.buildSharedQuery = function (query) {
    ...
    if (this.state.hasValue()) {
      var val = this.state.getValue();
      var rangeFilter = this.fieldContext.wrapFilter(RangeQuery(this.options.field, {
  ...
  RangeAccessor.prototype.buildOwnQuery = function (query) {
    ...
    return query.setAggs(FilterBucket.apply(void 0, [this.key,
...
var ResetSearchAccessor$$1 = (function (_super) {
  __extends(ResetSearchAccessor$$1, _super);
  function ResetSearchAccessor$$1(options) {
    if (options === void 0) { options = { query: true, filter: true, pagination: true }; }
    var _this = _super.call(this) || this;
    _this.options = options;
    return _this;
  }
  ResetSearchAccessor$$1.prototype.canReset = function () {
  ...
  ResetSearchAccessor$$1.prototype.performReset = function () {
    var query = this.searchkit.query;
    if (this.options.query) {
      this.searchkit.getQueryAccessor().resetState();
    }
    if (this.options.filter) {
      var filters = this.searchkit.getAccessorsByType(FilterBasedAccessor);
      forEach(filters, function (accessor) { return accessor.resetState(); });
      forEach(query.getSelectedFilters() || [], function (f) { return f.remove(); });
    }
    var paginationAccessor = this.searchkit.getAccessorByType(PaginationAccessor);
    if (this.options.pagination && paginationAccessor) {
      paginationAccessor.resetState();
    }
  };
  return ResetSearchAccessor$$1;
}(Accessor));

var ViewOptionsAccessor$$1 = (function (_super) {
  __extends(ViewOptionsAccessor$$1, _super);
  function ViewOptionsAccessor$$1(key, options) {
    var _this = _super.call(this, key) || this;
    _this.state = new ValueState();
    _this.options = options;
    return _this;
  }
  ViewOptionsAccessor$$1.prototype.getSelectedOption = function () {
    return find(this.options, { key: this.state.getValue() }) ||
      find(this.options, { defaultOption: true }) ||
      head(this.options);
  };
  ViewOptionsAccessor$$1.prototype.setView = function (key) {
  ...
  ViewOptionsAccessor$$1.prototype.search = function () {
...
var DynamicRangeAccessor = (function (_super) {
  ...
  DynamicRangeAccessor.prototype.buildOwnQuery = function (query) {
    var otherFilters = query.getFiltersWithoutKeys(this.key);
    return query.setAggs(FilterBucket.apply(void 0, [this.key,
      otherFilters].concat(this.fieldContext.wrapAggregations(StatsMetric(this.key, this.options.field)))));
  };
...
var PureRender = function (target) {
...
var AccessorManager = (function () {
  ...
  AccessorManager.prototype.setState = function (state) {
    forEach(this.getStatefulAccessors(), function (accessor) { return accessor.fromQueryObject(state); });
  };
  AccessorManager.prototype.notifyStateChange = function (oldState) {
    forEach(this.getStatefulAccessors(), function (accessor) { return accessor.onStateChange(oldState); });
  };
  ...
  AccessorManager.prototype.buildQuery = function () {
    forEach(this.getActiveAccessors(), function (accessor) { return accessor.beforeBuildQuery(); });
    return this.buildOwnQuery(this.buildSharedQuery(new ImmutableQuery()));
  };
  AccessorManager.prototype.setResults = function (results) {
    forEach(this.accessors, function (a) { return a.setResults(results); });
  };
  AccessorManager.prototype.resetState = function () {
    forEach(this.getStatefulAccessors(), function (a) { return a.resetState(); });
  };
...
var AxiosESTransport = (function (_super) {
  ...
  function AxiosESTransport(host, options) {
    ...
    _this.axios = axios.create(config);
    return _this;
  }
...
var VERSION$1 = '2.0.0-9';

var qs = require("qs");
var encodeObjUrl = function (obj) {
  return qs.stringify(obj, { encode: true, encodeValuesOnly: true });
};
var decodeObjString = function (str) {
  return qs.parse(str);
};
var createHistoryInstance = function () {
...
var SearchkitManager = (function () {
  ...
  SearchkitManager.prototype._searchWhenCompleted = function (location) {
    ...
    this.registrationCompleted.then(function () {
      _this.searchFromUrlQuery(decodeObjString(location.search.replace(/^\?/, "")));
    }).catch(function (e) {
  ...
  SearchkitManager.prototype.performSearch = function (replaceState, notifyState) {
    ...
    if (this.options.useHistory) {
      ...
      var url = this.options.getLocation().pathname + "?" + encodeObjUrl(this.state);
...
SearchkitManager.VERSION = VERSION$1;

var block$1 = require("bem-cn");
...
var RenderComponentPropType = PropTypes.oneOfType([
  function (props, propName, componentName) {
    if (isUndefined(props[propName]) || (props[propName]["prototype"] instanceof Component)) {
      return null;
    }
  },
  PropTypes.element,
...
function renderComponent(component, props, children) {
  ...
  var isReactComponent = (component["prototype"] instanceof Component ||
  ...
  if (isReactComponent) {
    ...
    return cloneElement(component, omitBy(props, isUndefined), children);
  }
...
export { Accessor, StatefulAccessor, CheckboxFilterAccessor, FacetAccessor, QueryAccessor, PaginationAccessor, HighlightAccessor, CustomHighlightAccessor, PageSizeAccessor, HierarchicalFacetAccessor, NumericOptionsAccessor$$1 as NumericOptionsAccessor, SortingAccessor, NestedFacetAccessor, NoFiltersHitCountAccessor, noopQueryAccessor, BaseQueryAccessor, NoopQueryAccessor, SuggestionsAccessor, AnonymousAccessor, FilterBasedAccessor, RangeAccessor, SourceFilterAccessor, ResetSearchAccessor$$1 as ResetSearchAccessor, ViewOptionsAccessor$$1 as ViewOptionsAccessor, HitsAccessor, DynamicRangeAccessor, ImmutableQuery, AggsContainer, TermsBucket, RangeBucket, ChildrenBucket, FilterBucket, NestedBucket, SignificantTermsBucket, GeohashBucket, HistogramBucket, FieldMetricFactory, CardinalityMetric, MinMetric, MaxMetric, AvgMetric, SumMetric, StatsMetric, TopHitsMetric, GeoBoundsMetric, AvgBucketPipeline, BoolMust, BoolMustNot, BoolShould, FilteredQuery, MatchPhrasePrefix, SimpleQueryString, QueryString, MatchQuery, MultiMatchQuery, HasChildQuery, HasParentQuery, NestedQuery, RangeQuery, TermQuery, TermsQuery, FieldContext, FieldContextFactory, EmbeddedFieldContext, ChildrenFieldContext, NestedFieldContext, block, FastClick, LoadingComponent, RenderComponentPropType, renderComponent, SearchkitComponent, SearchkitProvider, PureRender, PureRenderComponent, shallowEqualWithoutFunctions, shouldPureComponentUpdate, State, ArrayState, ObjectState, ValueState, LevelState, Utils, EventEmitter, SearchkitManager, encodeObjUrl, decodeObjString, createHistoryInstance, ESTransport, AxiosESTransport, MockESTransport, SearchRequest, AccessorManager, VERSION$1 as VERSION };
