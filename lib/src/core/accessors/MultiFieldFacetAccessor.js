    ...
    MultiFieldFacetAccessor.prototype.buildOwnQuery = function (query) {
        if (!this.loadAggregations) {
            ...
            return query.setAggs(__1.FilterBucket.apply(void 0, [this.uuid,
                query.getFiltersWithoutKeys(excludedKey)].concat(this.fieldContext.wrapAggregations(this.buildAggregations(query)))));
        }
    };
    MultiFieldFacetAccessor.prototype.buildAggregations = function (query) {
        var _this = this;
        return lodash_3.reduce(this.options.fields.reverse(), function (result, field, i) {
            return __1.TermsBucket(field, field, lodash_1.omitBy({
                size: _this.size,
